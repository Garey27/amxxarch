...
int __cdecl extract(const char *archive, const char* outputpath)
{
    ...
    path.append("/");
    path.append(dir);
    path.append("/addons/amxxarch/dlls/");
    path.append(kDllName);
    std::replace(path.begin(), path.end(), '/', '\\');
    lib.Load(const_cast<char*>(path.c_str()));
    ...
    FString archiveName = CmdStringToFString(archive);

    {
        ...
        else
        {
            ...
            extractCallbackSpec->Init(archive, UString(outputpath)); // second parameter is output folder path
